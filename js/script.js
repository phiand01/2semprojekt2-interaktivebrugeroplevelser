...
url: "https://iba-studie.net/login",
...
{id: 6,
title: "Flot klaret!",
description: "<p>Du undgik 'Praktik-Fælden'.</p><p>Ved at stoppe op undgik du 3 klassiske red flags:</p><p> 1. Tidspres (skal ske 'nu').</p><p> 2. Eksternt link til login.</p><p> 3. Forkert URL (iba-studie.net).",
...
const scene = story.find(s => s.id === sceneId);
    if (!scene) return; // Sikkerhedstjek
...
const urlBar = document.getElementById('url-sim');
    ...
    if (scene.id === 4) {
        titleElement.classList.add('hacked-title');
    } else {
        titleElement.classList.remove('hacked-title');
    }

    if (scene.url) {
        urlBar.innerHTML = `<p>Browser felt: <span>${scene.url}</span></p>`;
...
const btn = document.createElement('button'); btn.innerText = opt.text;
    ...
    if (opt.next !== undefined) {renderScene(opt.next);}});container.appendChild(btn);});}
